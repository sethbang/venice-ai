...
def print_models_info():
    ...
    table.add_row("Embedding model", "text-embedding-bge-m3")
...
def build_pytest_command(options: dict) -> List[str]:
    ...
    # Handle coverage: add pytest-cov flags
    if options.get("coverage", False):
        # Add pytest-cov coverage flags
        cov_source = options.get("cov_source", DEFAULT_COVERAGE_SOURCE)
        cmd.extend(["--cov", cov_source])
...
def run_tests(options: dict) -> int:
    ...
    # Use pytest command directly (pytest-cov will handle coverage if enabled)
    cmd = pytest_cmd
    ...
    # Coverage reports are now handled by pytest-cov during test execution
    if options.get("coverage", False):
        # Check if coverage.xml was created by pytest-cov
        if Path("coverage.xml").exists():
            console.print("coverage.xml generated successfully by pytest-cov.", style=Style.SUCCESS)
        else:
            console.print("coverage.xml was NOT generated by pytest-cov.", style=Style.WARNING)
