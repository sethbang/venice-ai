...
This is a different surface from x402 prepaid balance (`client.x402.balance(...)`), which has its own shape — see `venice-ai-x402/references/balance-and-topup.md`. Don't confuse the two.
...
There is no `response.audio`, no `response.bytes()`, and no `response.read()`. The raw bytes live on `response.content`; `save()` and `iter_bytes()` are the other accessors. When `stream=True` the return type changes to `AsyncIterator[bytes]` — see `venice-ai-multimodal/references/audio-tts-stt.md`.
...
- `venice-ai/references/billing.md` — full `client.billing.*` surface
- `venice-ai-multimodal/references/audio-tts-stt.md` — voice scoping, formats, streaming
- `venice-ai-x402/references/balance-and-topup.md` — distinct from `client.billing.get_balance()`
- `venice-ai/references/headers-and-metadata.md` — `_response`, headers, rate limits, deprecation info
