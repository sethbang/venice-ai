...
This skill covers the **generation** side: producing images, speech, transcripts, music, and video with Venice. For chat and vision-as-input (multimodal chat), see the `venice-ai` skill.

The cardinal rule from `venice-ai` carries over: **never hardcode model IDs**. Resolve via `client.models.resolve_image()`, `resolve_tts()`, `resolve_asr()`, `resolve_music()`, `resolve_video()`, or `resolve_cheapest_video()`.
