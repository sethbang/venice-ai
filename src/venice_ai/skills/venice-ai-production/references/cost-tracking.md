...
- **Threading raw responses to `tracker.track()` from streaming runs** — pass `stream.final_response` (which IS a `ChatCompletionResponse`), not individual chunks. `final_response` is only populated when you use `collect_with_deltas()` or `collect()` — see `venice-ai/references/streaming.md`.
...
- `venice-ai-x402/SKILL.md` — top-up flow when the prepaid balance runs out.
- `venice-ai/references/headers-and-metadata.md` — full surface of `_response`-derived properties including `balance_info`.
